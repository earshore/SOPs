...
| R4 | Keyword Hunter | **Pass (automated)** | smoke keyword-hunter input + 空态 + **ownership**（`wb-theme-cyan` / sidebar 回退） |
...
| R4-C2 | KH ownership banner/sidebar | **Pass (automated)** class + **Needs manual browser** 质感 | 活模板 **`wb-theme-cyan`**（2026-08-06 由 rose 统一收敛）；smoke 在 minimal（及 dark×minimal）后仍断言类名；渐变质感仍需人工 |
| R4-C3 | 侧栏 KH 目录色 | **Pass (automated)** 回退路径 + **Needs manual browser** | smoke 可回退 `sidebar-theme-cyan`；扫读仍人工 |
...
| R7-C5 | 切换不改侧栏/banner | **Pass (automated)** KH 抽检 + **Needs manual browser** 全站 | smoke：minimal 后 KH `wb-theme-cyan`；其他路由仍需肉眼 |
...
| X2 | Ownership 不变 | **Pass (automated)** KH cyan 抽检 + **Needs manual browser** 全站 |
...
| ☐ | **dark × minimal 烟测复核**（自动化已有） | 设置切 deep + 极简后打开 KH：`<html data-appearance=minimal data-color-mode=dark class~dark>`；cyan banner/sidebar 类仍在 | 轴互擦；ownership 类消失（契约回归） |
...
| KH ownership class 在 minimal 后 | smoke **有** `wb-theme-cyan` / `sidebar-theme-cyan` |
...
| 10–14 | 仍 minimal：打开 Keyword Hunter | banner/侧栏 **cyan 归属**（`wb-theme-cyan` / 侧栏 cyan）**不变**；仅 token 化壳层可偏 slate | banner 变灰/变蓝跟 Appearance |
...
| 5 | 10–14 | KH `/#/app-center/keyword-hunter/…` | **cyan** ownership 不变（`wb-theme-cyan` / sidebar） |
...
**Ownership 必抽（X2）**: KH cyan · PPC hero · MA indigo · Deep Chat terracotta。  
**双轴必抽（X5）**: Settings dark + 保留/ 切换 Appearance；可选再开 KH 复核 cyan class。
...
X2 ownership (KH cyan / PPC / MA / DeepChat terracotta): Pass/Fail
