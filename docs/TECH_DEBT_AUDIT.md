...
| 已完成 | 审计报告产物                          | 生成 `docs/css-module-analysis-report.md`、`tests/quality/tech-debt-2026-07-02.json`、`tests/quality/tech-debt-2026-07-02.html` 和最新复杂度报告 `complexity-report-2026-07-02T16-12-10.*`。          |
...
| XSS 高危门禁             | `npm run xss:gate`                                                                                                                                                                                                                                                                                                                                                                                                    | 467 个源文件，0 风险点；已审计安全跳过 13 处、清空 DOM 跳过 6 处                                                                     |
| 循环依赖                 | `npm run circular:check`                                                                                                                                                                                                                                                                                                                                                                                              | 485 个文件，0 循环依赖，madge 输出 3 个解析 warning                                                                                  |
...
| 复杂度分析               | `npm run code:analyze:complexity`                                                                                                                                                                                                                                                                                                                                                                                     | 690 个文件，12,920 个函数，177 个过长函数、86 个高复杂度函数，234 个问题函数                                                         |
| 技术债扫描               | `npm run tech-debt:scan`                                                                                                                                                                                                                                                                                                                                                                                              | 397 个文件，98,660 行，0 issue，债务比率 0.00%                                                                                       |

## 剩余技术债务

| ID    | 优先级 | 债务                          | 当前证据                                                                                                                                                 | 验收条件                                                                                                  |
| ----- | ------ | ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| TD-04 | P2     | `src/` 复杂度热点             | `quality:check` 显示平均复杂度 19.4，最大复杂度 261，175 个文件超过阈值；`code:analyze:complexity` 显示 234 个问题函数，其中 177 个过长、86 个高复杂度；当前 `src/` 热点集中在 SafeModuleLoader、Scraper import、More prompts、Keyword Hunter 等局部函数。 | 按 top 文件拆分长函数和深嵌套，补回局部测试，复杂度指标下降且 ESLint warning gate 保持 0。                |
...
2. **继续 TD-04 `src/` 复杂度热点**：按最新复杂度报告逐个处理 `loadModule`、`showMarketplaceSelectionModal`/`mergeProducts`、`registerWindowActions`、`highlightText`/`renderCopyDisplay` 等局部函数。每批只拆一个业务边界，验收为目标函数退出复杂度报告、相关专项测试通过、`type-check` 和 `lint:warning-gate` 通过。
...
- [ ] `src/common/infrastructure/SafeModuleLoader.ts`：继续拆分 `loadModule` 的缓存命中、动态导入、模块解析和错误收尾。
