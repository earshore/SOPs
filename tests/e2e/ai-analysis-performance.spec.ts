...
const PERFORMANCE_BASELINE = {
  ...
  test.describe('页面加载性能', () => {
    ...
    test('ASIN 选择响应时间应该 < 100ms', async ({ page }) => {
      ...
      if (selectDelay === -1) {
        console.log('⚠️ ASIN 复选框未找到，跳过测试');
        test.skip();
        return;
      }
      
      console.log(`📊 ASIN 选择响应时间: ${selectDelay.toFixed(2)}ms`);
      console.log(`   基线: ${PERFORMANCE_BASELINE.interactionDelay}ms`);
      
      // 验证：选择响应时间应该 < 100ms
      expect(
        selectDelay,
        `ASIN 选择响应时间应该 < ${PERFORMANCE_BASELINE.interactionDelay}ms，实际: ${selectDelay.toFixed(2)}ms`
      ).toBeLessThan(PERFORMANCE_BASELINE.interactionDelay);
    });

    test('分析目标选择响应时间应该 < 100ms', async ({ page }) => {
      await aiAnalysis.navigate();
      ...
      if (selectDelay === -1) {
        console.log('⚠️ 分析目标复选框未找到，跳过测试');
        test.skip();
        return;
      }
      
      console.log(`📊 分析目标选择响应时间: ${selectDelay.toFixed(2)}ms`);
      console.log(`   基线: ${PERFORMANCE_BASELINE.interactionDelay}ms`);
      
      // 验证：选择响应时间应该 < 100ms
      expect(
        selectDelay,
        `分析目标选择响应时间应该 < ${PERFORMANCE_BASELINE.interactionDelay}ms，实际: ${selectDelay.toFixed(2)}ms`
      ).toBeLessThan(PERFORMANCE_BASELINE.interactionDelay);
    });
    ...
    test('全选/清空操作响应时间应该 < 100ms', async ({ page }) => {
      await aiAnalysis.navigate();
      ...
      if (selectAllDelay === -1) {
        console.log('⚠️ 全选按钮未找到，跳过测试');
        test.skip();
        return;
      }
      
      console.log(`📊 全选操作响应时间: ${selectAllDelay.toFixed(2)}ms`);
      console.log(`   基线: ${PERFORMANCE_BASELINE.interactionDelay}ms`);
  ...
  test.describe('渲染性能', () => {
          ...
          for (let i = 0; i < iterations; i++) {
            container.innerHTML = `
              <div class="result-card">
                ...
                <div class="stats">
                  <span>数据点: ${i * 10}</span>
                  <span>置信度: ${(Math.random() * 100).toFixed(2)}%</span>
                </div>
              </div>
            `;
          }
    ...
    test('结果卡片列表渲染性能应该可接受', async ({ page }) => {
      await aiAnalysis.navigate();
          ...
          const html = items
            .map(item => `
              <div class="result-card">
                ...
                <p>${item.summary}</p>
                <div class="stats">
                  <span>数据点: ${item.stats.dataPoints}</span>
                  <span>置信度: ${item.stats.confidence}%</span>
                </div>
              </div>
            `)
            .join('');
      ...
      expect(
        ...
        `结果卡片列表渲染总时间应该 < 100ms，实际: ${listRenderPerf.totalTime.toFixed(2)}ms`
      ).toBeLessThan(100);
    });
  });
