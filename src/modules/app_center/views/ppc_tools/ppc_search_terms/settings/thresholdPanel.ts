...
export function toggleThresholdPanel(container: HTMLElement): void {
  const toggle = getButton(container, 'ppc-threshold-toggle');
  if (!toggle) return;
...
function setThresholdPanelExpanded(container: HTMLElement, isExpanded: boolean): void {
  const toggle = getButton(container, 'ppc-threshold-toggle');
  const body = getElement(container, 'ppc-threshold-body');
  if (!toggle || !body) return;

  toggle.setAttribute('aria-expanded', String(isExpanded));
  body.classList.toggle('hidden', !isExpanded);
  const label = getElement(container, 'ppc-threshold-toggle-label');
