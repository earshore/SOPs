...
test.describe('页面加载性能', () => {
  test('页面加载时间应该在基线范围内', async ({ page }) => {
    ...
    console.log(`📊 Promptlab 页面加载时间: ${loadTime.toFixed(2)}ms`);
    ...
    // 计算与基线的差异百分比
    const deviation =
      ((loadTime - PERFORMANCE_BASELINE.pageLoadTime) / PERFORMANCE_BASELINE.pageLoadTime) * 100;
  ...
  test('模块初始化时间应该合理', async ({ page }) => {
    ...
    expect(initTime, 'Promptlab Alpine 组件应该已初始化').toBeGreaterThanOrEqual(0);
    ...
    const deviation =
      ((initTime - PERFORMANCE_BASELINE.moduleInitTime) / PERFORMANCE_BASELINE.moduleInitTime) *
      100;
    console.log(`   偏差: ${deviation > 0 ? '+' : ''}${deviation.toFixed(2)}%`);
  });
  ...
  test('首次渲染时间应该在基线范围内', async ({ page }) => {
    ...
    // 等待关键元素渲染
    await page.waitForSelector('#card-product-dna', { state: 'visible' });
    await page.waitForSelector('#card-analysis', { state: 'visible' });
    await page.waitForSelector('#card-strategy', { state: 'visible' });
    ...
    const deviation =
      ((renderTime - PERFORMANCE_BASELINE.firstRenderTime) / PERFORMANCE_BASELINE.firstRenderTime) *
      100;
    console.log(`   偏差: ${deviation > 0 ? '+' : ''}${deviation.toFixed(2)}%`);
  });
});
...
test('输入响应时间应该 < 100ms', async ({ page }) => {
  ...
  const inputDelay = await page.evaluate(() => {
    return new Promise<number>(resolve => {
      const input = document.querySelector('#lab-keywords-tier1') as HTMLInputElement;
      if (!input) {
        resolve(-1);
        return;
      }
  ...
  if (inputDelay === -1) {
    console.log('⚠️ 输入元素未找到，跳过测试');
    test.skip();
    return;
  }

  console.log(`📊 输入响应时间: ${inputDelay.toFixed(2)}ms`);
  console.log(`   基线: ${PERFORMANCE_BASELINE.interactionDelay}ms`);

  // 验证：输入响应时间应该 < 100ms
  expect(
    inputDelay,
    `输入响应时间应该 < ${PERFORMANCE_BASELINE.interactionDelay}ms，实际: ${inputDelay.toFixed(2)}ms`
  ).toBeLessThan(PERFORMANCE_BASELINE.interactionDelay);
});

test('下拉框切换响应时间应该 < 100ms', async ({ page }) => {
  await promptlab.navigate();

  // 测量下拉框切换响应时间
  const selectDelay = await page.evaluate(() => {
    return new Promise<number>(resolve => {
      const select = document.querySelector('#lab-target-market') as HTMLSelectElement;
      if (!select) {
        resolve(-1);
        return;
      }
  ...
  if (selectDelay === -1) {
    console.log('⚠️ 下拉框元素未找到，跳过测试');
    test.skip();
    return;
  }

  console.log(`📊 下拉框切换响应时间: ${selectDelay.toFixed(2)}ms`);
  console.log(`   基线: ${PERFORMANCE_BASELINE.interactionDelay}ms`);

  // 验证：下拉框切换响应时间应该 < 100ms
  expect(
    selectDelay,
    `下拉框切换响应时间应该 < ${PERFORMANCE_BASELINE.interactionDelay}ms，实际: ${selectDelay.toFixed(2)}ms`
  ).toBeLessThan(PERFORMANCE_BASELINE.interactionDelay);
});
...
test('按钮点击响应时间应该 < 100ms', async ({ page }) => {
  ...
  const clickDelay = await page.evaluate(() => {
    return new Promise<number>(resolve => {
      const button = document.querySelector('#btn-generate-prompt') as HTMLButtonElement;
      if (!button) {
        resolve(-1);
        return;
      }
...
test('页面内存占用应该在基线范围内', async ({ page }) => {
  ...
  const deviation =
    ((memoryUsage - PERFORMANCE_BASELINE.memoryUsage) / PERFORMANCE_BASELINE.memoryUsage) * 100;
  console.log(`   偏差: ${deviation > 0 ? '+' : ''}${deviation.toFixed(2)}%`);
...
test('多次操作后内存不应持续增长（内存泄漏检测）', async ({ page }) => {
  ...
  for (let i = 0; i < 10; i++) {
    await promptlab.fillProductDNA({
      targetMarket: 'English',
      tier1Keywords: `test product ${i}`,
      tier2Keywords: `test keywords ${i}`,
    });
...
test('列表渲染性能应该可接受', async ({ page }) => {
  ...
  console.log(`📊 列表渲染性能测试:`);
  console.log(`   项目数量: ${listRenderPerf.itemCount}`);
  console.log(`   总时间: ${listRenderPerf.totalTime.toFixed(2)}ms`);
  console.log(`   平均时间: ${listRenderPerf.avgTime.toFixed(4)}ms/项`);

  // 验证：总渲染时间应该 < 100ms（100 项）
  expect(
...
test.describe('Web Vitals 指标', () => {
  test('应该收集并验证 Web Vitals 指标', async ({ page }) => {
    ...
    if (vitals.fcp !== null) {
      console.log(
        `   FCP: ${vitals.fcp.toFixed(2)}ms (基线: ${PERFORMANCE_BASELINE.lighthouse.fcp}ms)`
      );
      expect(
        vitals.fcp,
...
test.describe('性能回归检测', () => {
  test('应该生成性能报告并与基线对比', async ({ page }) => {
    ...
    console.log(
      `   ✓ 首屏渲染时间 < 4s: ${metrics.pageLoad < PERFORMANCE_BASELINE.firstRenderTime ? '通过' : '失败'}`
    );
    console.log(
      `   ✓ 页面加载时间 < 4s: ${metrics.pageLoad < PERFORMANCE_BASELINE.pageLoadTime ? '通过' : '失败'}`
    );
    ...
    console.log('\n结论:');
    if (metrics.deviation < 5) {
      console.log('   ✅ Promptlab 模块迁移后性能无退化');
