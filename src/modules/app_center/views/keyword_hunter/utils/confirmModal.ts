...
function createBackdrop(modalId: string): HTMLDivElement {
  ...
  backdrop.id = modalId;
  backdrop.className =
    'kh-confirm-modal-backdrop fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[60] flex items-center justify-center fade-in';
  return backdrop;
}
...
function buildDontAskMarkup(modalId: string, ignoreKey: string): string {
  ...
  return `
        <label class="flex items-center gap-2 text-xs text-slate-500 mb-4 cursor-pointer">
            <input type="checkbox" id="dont-ask-again-${modalId}" class="kh-confirm-modal-checkbox rounded border-slate-300">
            <span>不再提示</span>
        </label>
    `;
}
...
function buildModalContent(
  ...
  const variantClass = request.isDestructive
    ? 'kh-confirm-modal--danger'
    : 'kh-confirm-modal--theme';
  const iconClass = request.isDestructive
    ? 'fas fa-exclamation-triangle'
    : 'fas fa-circle-question';

  return `
        <div class="kh-confirm-modal ${variantClass} bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden transform scale-100 transition"
            role="dialog"
            aria-modal="true"
            aria-labelledby="${titleId}"
            aria-describedby="${descriptionId}">
            <div class="kh-confirm-modal-header p-5 text-white">
            ...
            <div class="p-6">
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="button" id="btn-confirm-${modalId}" class="kh-confirm-modal-confirm min-h-10 px-5 py-2 text-white rounded-lg text-sm font-bold shadow-md transition-transform transform active:translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2">
...
function cleanupModal(
  ...
  try {
    ...
    console.error('[KeywordHunter] 清理确认对话框失败:', error);
  }
}
...
function mountConfirmModal(request: ConfirmModalRequest): void {
  ...
  if (!elements.btnConfirm || !elements.btnCancel) {
    console.error('[KeywordHunter] 确认对话框渲染不完整，已自动关闭');
    finish(false);
    return;
  }
