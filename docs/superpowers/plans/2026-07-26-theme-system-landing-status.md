...
**日期**: 2026-07-26（refresh @ HEAD `9bdc7ae9` · D6 #25 AppModal `fb5384d6` · D2 #6 deferred `21bec249` · D5 inventory `5bde2abb` · XO checklist `a72046bb` · adjacent settings TD-SET `aba5a951`→`9bdc7ae9` · Visual Yellow · Human XO first）  
**范围**: `main` ahead of `sops/main`； residual 主题波 + D6 #24 confirmModal + **#25 AppModal** + **D2 #6 docs deferral** + D5 residual inventory + XO checklist tighten；adjacent settings TD-SET scaffold/e2e **已在** tip（full `isNavTargetCurrent` scroll-spy bind body may still be dirty outside tip）  
...
| 层                 | 灯         | 一句话                                                                                                                                                                                                                                           |
| ------------------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Code / 契约**    | **Green**  | tip `9bdc7ae9`: D6 #18–**25**（**#25 AppModal** @ `fb5384d6`）· D12×**24** · D2 #2–#5 + **#6 deferred** @ `21bec249` · D5 residual inventory @ `5bde2abb` · XO checklist @ `a72046bb` · D8 soft · shell nav/settings focus |
...
**一句话**: residual #18–24 + **#25 AppModal** primary soft chrome **已在** tip `9bdc7ae9`；D2 radius long-tail **#6 deferred — diminishing returns**；体验关闸仍只靠 **人工 XO**。smoke 用 `npm run test:e2e:smoke`（4173）。
...
| **5** D6 业务页分期            | 业务 `blue-*` 降 | **Partial (samples)**                         | 业务 ~900+ 仍 Informational 基线；**样本 through #16 + residual #18–25**: KH…#9 + **#10 SOPs** + **#11 NPI** + **#12 Restricted Words** + **#13 AMZ Hub nav**（ownership orange / `module-accent`）+ **#14 email_templates** + **#15 qa_maintenance** + **#16 PromptLab DNA/autoPopulate** + **#18 Scraper / #19 AI / #20 PPC residual** @ `a05b12c8` + **#21 PromptLab residual focus** @ `f761e4e5` + **#22 SafeModuleLoader primary CTA** @ `1711eec1` + **#23 Home splash/floating CTA** @ `cbad435b` + **#24 confirmModal primary chrome** @ `3b947dc6` + **#25 AppModal title/accent soft chrome** @ `fb5384d6`；Skills #6 / Deep Chat #7；phase/tab/step 多色与 SEO 卡 **保留**；壳层 megaMenu **13**                                                                |
...
| 业务 `blue-*`（D6）              | ~900+ 行级基线                                                     | Phase 5；**样本 through #16 + residual #18–25**: KH…#9 + **#10 SOPs** + **#11 NPI** + **#12 Restricted Words** + **#13 AMZ Hub nav** + **#14 email_templates** + **#15 qa_maintenance** + **#16 PromptLab DNA** + **#18–20 residual MA/PPC** @ `a05b12c8` + **#21 PromptLab residual focus** @ `f761e4e5` + **#22 SafeModuleLoader** @ `1711eec1` + **#23 Home splash/floating CTA** @ `cbad435b` + **#24 confirmModal** @ `3b947dc6` + **#25 AppModal** @ `fb5384d6` + Skills #6 / Deep Chat #7；`--shadow-primary*` 跟 primary；**非**全量清零；不阻塞 code Green |
| Ownership Role 表                | **19 roles 文档**                                                  | `2026-07-26-ownership-role-palette-map.md`；实现未绑 `data-ownership-role` 全站                                                                                                                     |
| Workbench colorSchemes helpers   | **API + unit + 首批调用点 + misuse audit clean**                   | helpers 已导出；**AI / PromptLab / Scraper / KH report / AI JSON icons** 已迁；**§5.2**: 0 production entry-helper 误用 / 0 非 entry `scale-110`；**`getWorkbenchCardClasses` still 0 production callers**（re-audit @ HEAD `147aa90c`；候选模块见 §5.2）                                                    |
...
| **D6**  | P1 可见面 | **Shell locked / biz samples**      | 壳层 megaMenu **13** = **有意 Ownership**；**样本 through #16 + residual #18–25**: KH…#9 + **#10 SOPs** + **#11 NPI** + **#12 Restricted Words** + **#13 AMZ Hub nav**（hover/active → ownership orange / `module-accent`）+ **#14 email_templates** + **#15 qa_maintenance** + **#16 PromptLab DNA/autoPopulate** + **#18 Scraper / #19 AI / #20 PPC residual** @ `a05b12c8` + **#21 PromptLab residual focus** @ `f761e4e5` + **#22 SafeModuleLoader primary CTA** @ `1711eec1` + **#23 Home splash/floating CTA** @ `cbad435b` + **#24 confirmModal primary chrome** @ `3b947dc6` + **#25 AppModal title/accent soft chrome** @ `fb5384d6` + Skills/DeepChat…；其余 Informational 长尾 |
...
**Migrated earlier residual (code @ tip `9bdc7ae9` lineage):**
...
| Settings (`systemSettings.*`) | TD-SET scaffold/e2e @ `aba5a951`→`9bdc7ae9`；theme board **does not** claim scroll-spy visual Pass | **Adjacent non-theme** |
...
**D4 residual — `getWorkbenchCardClasses` production callers**: **0** (re-audit HEAD `147aa90c`; prior notes `d557491b` / `904f83d3`).

Grep / call-site evidence (re-skim @ `147aa90c`):
- `getWorkbenchCardClasses(` → definition in `src/common/constants/colorSchemes.ts` + unit in `colorSchemes.test.ts` only (**0** production callers).
- Production `getCardClasses(` / marketing card motion on tool chrome: **none** (megaMenu keep entry lift).
- Production `getWorkbenchIconContainerClasses` sites still only: AI Analysis (`AlpinePanel` selection + JSON icons), PromptLab section icons, Scraper strategy icon, KH analysis report icon.
- Co-located shells re-checked for **one** natural scheme-border Tailwind card migrate target:
  - **AI Analysis** (`AlpinePanel` + template): hero/target/result chrome = primary CSS vars, slate neutrals, status/result tone maps — not `group/card` scheme-border shells.
  - **Scraper** (panel/template + product/review cards): neutral/primary-token borders + expanded ring; purple hover accents only — not full scheme card chrome.
  - **PromptLab** (section icons + report dimension shells): slate / token panels; list rows ≠ scheme card shells.
  - **KH analysis** report icon only; surface cards remain CSS workbench radius / slate shells.
  - **amz_hub** multi-color HTML cards = story/content surfaces — **do not** force-migrate.
...
**Code fix this wave**: **none** (docs-only re-audit; card helper still has **0** production callers; **no** single natural scheme-border migrate site).
...
| #     | Wave                                              | 范围                                                                                                                         | 验证                                                                       |
| ----- | ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| **1** | **Human XO（始终优先 · 阻塞体验关闸）**           | 30 min 浏览器脚本 + 记录模板；X1/X2/X5 + 可选 Skills / Deep Chat shell / **#10 SOPs** / **#11 NPI** / **#13 Hub nav orange** / **#14 email_templates** / **#15 qa_maintenance** / **#16 PromptLab DNA** / **#18–25 residual MA/PPC/PromptLab + SafeModuleLoader + Home CTA + confirmModal + AppModal**                           | 矩阵勾选 + XO status 结论；**仍不宣称 visual Pass**                        |
...
| **3** | **D6 下一业务样本（可并发 · 低优先）**            | #10–#16 + residual #18–**25** **已落**（含 **#13 Hub nav** @ `904f83d3` + #14–#15 @ `2f8d98ab` + **#16** @ `f99b0fc8` + **#18–20** @ `a05b12c8` + **#21** @ `f761e4e5` + **#22** @ `1711eec1` + **#23 Home CTA** @ `cbad435b` + **#24 confirmModal** @ `3b947dc6` + **#25 AppModal** @ `fb5384d6`）；下一页 primary/chrome 样本（非 900+ 清零）；**禁止**重写 terracotta / phase 语义蓝 / RW 教学 INFO 卡 / Hub 归属橙 | hardcode gate 不升；**仍不宣称 visual Pass**                               |
...
```
9bdc7ae9 test(settings): stabilize TD-SET-05 e2e assertion for KH deep-link  ← HEAD
...
02ab689f docs(theme): refresh landing tip after confirmModal
be4329f1 style(theme): format confirmModal and D8 test guards
3b947dc6 feat(theme): migrate confirmModal primary chrome
b56e2969 docs+test: close TD-DOC-01, TD-SET-02, TD-TEST-02 without theme touch
a20fb500 docs(theme): refresh landing tip after D8 type-check fix
52795047 fix(theme): guard optional menu categories in D8 tests
...
- D6 #22 SafeModuleLoader primary CTA @ `1711eec1`
- D6 #23 Home splash/floating primary CTA @ `cbad435b`
- D6 #24 confirmModal primary chrome @ `3b947dc6`（format @ `be4329f1`）
- D6 #25 AppModal title/accent soft chrome @ `fb5384d6`
- D12 scaffold **24**（+ PromptLab + Skills + NPI）@ `023b8952`
- D2 workbench radius long-tail #5 @ `b85cd597`；**#6 deferred** docs inventory @ `21bec249`
- D5 shared-shell focus residual inventory @ `5bde2abb`
...
- Adjacent settings TD-SET scaffold/e2e @ `aba5a951` → stabilize @ `9bdc7ae9`（theme board tip；**not** visual Pass）
- D4 card helper still **0** production callers
- full prebuild / smoke **Green 再次**（不代表 visual Pass）
- **Visual Yellow** · **Human XO first**（仍未签）
- **无** new D6 样本 beyond residual #18–25（本轮 tip 含 **#25 AppModal**）
