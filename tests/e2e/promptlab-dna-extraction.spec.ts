...
test('应该能够从分析报告自动提取产品 DNA', async ({ page }) => {
  ...
  console.log(
    `     - 目标受众: ${dna.audience.substring(0, 50)}${dna.audience.length > 50 ? '...' : ''}`
  );
...
test('应该在确认后覆盖现有内容', async ({ page }) => {
  ...
  const dna = await promptlab.getAutoFilledDNA();
  const isUpdated =
    dna.audience !== '旧的受众内容' || dna.usps !== '旧的卖点内容' || dna.specs !== '旧的参数内容';
...
test.describe('完整流程测试', () => {
  test('应该完成从提取 DNA 到生成 Prompt 的完整流程', async ({ page }) => {
    ...
    const sectionsCount = await promptlab.page
      .locator('#report-sections-container input[type="checkbox"]')
      .count();
    if (sectionsCount > 0) {
    ...
    // 验证：Prompt 已生成
    const hasPrompt = await promptlab.page
      .locator('#final-prompt-output')
      .inputValue()
      .then(v => v.length > 0);
