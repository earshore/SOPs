...
- `v3.0.12`（2026-08-08）为当前 GA：定稿 `v3.0.12-rc.1`…`rc.10`（AI 分析推理联动与动态耗时估算、Deep Chat 正文净化/流式去重、数据采集合并/导入新的、ModelSelect 组件统一、门禁加固）；Latest 指向 `v3.0.12`，回滚基线 `v3.0.11`。
...
- 当前候选线为 `v3.1.0-rc.3-rc.*`（2026-08-09 开启：AI 分析取消/运行感知、总览优化；2026-08-10 `rc.2` 会话设置加固、`rc.3` 请求链路可靠性与推送门禁；GA 后将关闭本线并进入下一版本线）。
- `v3.1.0-rc.3`（2026-08-10）为当前 Pre-release 候选：Deep Chat 跨会话串线/删除复活/预算预检/KH 推送误拦/双提交/输入持久化；**Pre-release**，Latest 仍为 `v3.0.12`。
