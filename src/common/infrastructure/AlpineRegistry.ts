...
export class AlpineRegistry {
  ...
  private static readonly LOG_METHODS: Record<RegistryLogLevel, RegistryLogMethod> = {
    debug: (...data: unknown[]) => nativeLoggerConsole.log(...data),
    info: (...data: unknown[]) => nativeLoggerConsole.log(...data),
    warn: (...data: unknown[]) => nativeLoggerConsole.warn(...data),
   ...
   * ```
   */
  public register(name: string, factory: () => unknown, dependencies: string[] = []): void {
  ...
  public init(): void {
    ...
    if (!this.isAlpineAvailable()) {
      ...
      throw new SystemError('Alpine.js 未就绪', 'ALPINE_NOT_READY', {
        module: 'AlpineRegistry',
        action: 'init',
      });
    }
  ...
  private resolveDependencies(): AlpineComponent[] {
    ...
    const visit = (component: AlpineComponent): void => {
      ...
      if (visiting.has(component.name)) {
        ...
        this.log('error', error);
        throw new SystemError(`检测到循环依赖: ${component.name}`, 'ALPINE_CIRCULAR_DEPENDENCY', {
          module: 'AlpineRegistry',
          action: 'resolveDependencies',
          componentName: component.name,
          visitingChain: Array.from(visiting),
        });
      }
  ...
  private registerToAlpine(component: AlpineComponent): void {
    ...
    if (!this.isAlpineAvailable()) {
      ...
      this.log('error', error);
      throw new SystemError('Alpine.data 方法不可用', 'ALPINE_DATA_UNAVAILABLE', {
        module: 'AlpineRegistry',
        action: 'registerToAlpine',
        componentName: component.name,
      });
    }
  ...
  private isAlpineAvailable(): boolean {
    return (
      typeof window !== 'undefined' && window.Alpine && typeof window.Alpine.data === 'function'
    );
  }
