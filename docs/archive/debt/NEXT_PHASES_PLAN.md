...
### 次序 14：smoke baseline 断言层修复（**已完成**，`2c640226`，2026-08-15，`SMOKE_BASELINE_FIX_PLAN.md` 第 6 节）

rc.4 后全浏览器冒烟固定 3 例失败（NPI lifecycle 像素断言 firefox/webkit dpr 尺寸不匹配 + core routes webkit 时序误判），排查结论为基线既有缺陷（历次冒烟清单一致、非回归），断言层 `captureStableRegion` 与浏览器 dpr 耦合为根因。**执行结果**：Step 1 `page.screenshot({ scale: 'css' })` 消除尺寸差异后，firefox（3.2%）/webkit（2.7%）仍存在真实像素级渲染差异，兜底项升级为 per-engine 基线（`npi-table-status-colors-{light,dark}-{firefox,webkit}.png`，chromium 复用原名）；core routes webkit 实测定位为 Deep Chat 路由 390px 稳态文本确为 33 chars（webkit 布局折叠的真实差异，非时序），新增路由级 `minContentLength` 阈值 + `expectRouteReady` 双 rAF 稳定化。验收：smoke 93/93 全绿 + build + ci:quality 20 项全绿（semantic 2128/2128、modules 0/0、shell 24/24、settings-scale 1199/1200）。TD-E2E-01 专项闭环，视觉回归抽检现覆盖三引擎。
