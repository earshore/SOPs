...
| **PL-O3** | 密度与变体可测、可复用，不另起全局 token 体系；工具策略 tool-page 统一单 pref-list |
...
| **C** | Composite / Step Card | 多字段流程、可折叠高级项、成组验证   | LLM 四步、连接测试、危险分桶操作；代理连接表单 | 卡片/步骤容器；内部可局部嵌 A   |
...
### 5.4 工具策略 tool-page 合同（单 pref-list）

**规则：** 每个 `settings-tool-page` / `settings-tool-l3__body` **只保留一个** `settings-pref-list`。

| 内容 | 写法 |
| ---- | ---- |
| 模型 / toggle / number / select | 同一 list 内的标准 `settings-pref-row`，行间 `settings-pref-divider` |
| 短数字参数（超时、并发、Token） | **全宽 A 行**（左 title+hint，右 control），**不再**使用 `settings-pref-metrics` |
| 代理连接双字段表单 | 仍可走 C 形态（`settings-proxy-form`）；其后采集数字簇并入 pref-list |
| 外框 | 若 body 只有一个 pref-list，CSS 去掉空 tool-page chrome |

**历史决策：** Aᵐ metrics lattice 已回退。多列格在窄设置面板里会把中文标题挤成省略号，且与「同一 body 一种 list 格式」冲突。

**产品放置提醒：** 全局「应用策略预案」是 tool 区顶层 A 行（`#settings-runtime-presets`），含 **默认 / 稳定 / 速度 / 成本**；hint `LLM / 采集 / 分析 / Deep Chat / PPC`；**不是**「通用 AI」专属，也不是「数据采集」专属。
