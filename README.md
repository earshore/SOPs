...
| **当前 Pre-release 候选**    | `v3.0.12-rc.10` | AI 分析推理联动与动态耗时估算 + Deep Chat 正文净化/流式去重 + 数据采集合并/覆盖导入；**非** Latest |
...
- **数据采集**：产品导入拆分为**合并导入**（保留未覆盖 ASIN、相同 ASIN 仅并入评论）与**覆盖导入**（清空替换 + 确认弹窗）；双按钮 + 双隐藏 input 可访问性同步。
