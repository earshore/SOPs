...
function createSettingsPanel(): SettingsPanelData {
  return {
    ...
    activeModelInfo(): { id: string; name?: string } | null {
      ...
      const m = this.llm.models.find(
        (x) => (typeof x === "string" ? x : x.id) === this.llm.model,
      );
    ...
    async openPerformanceMonitor(): Promise<void> {
      try {
        const { performanceMonitor } =
          await import("../../common/devtools/PerformanceMonitor");
    ...
    async loadProviderConfig(provider: string): Promise<void> {
      ...
      try {
        ...
        console.warn(
          "[Settings] Failed to load encrypted API key, using fallback:",
          error,
        );
        // 兼容旧数据: 如果加密读取失败,尝试读取明文(迁移期)
        this.llm.apiKey =
          savedConfig && "apiKey" in savedConfig
            ? savedConfig.apiKey || ""
            : "";
      }

      // Models: Use saved or default
      const rawModels: Array<string | { id: string }> =
        savedConfig &&
        "models" in savedConfig &&
        savedConfig.models &&
        savedConfig.models.length > 0
          ? savedConfig.models
          : config?.models || [];
    ...
    async fetchModels(): Promise<void> {
      ...
      try {
        ...
        if (
          ["llmgateway", "openai", "deepseek", "moonshot", "qwen"].includes(
            provider,
          )
        ) {
    ...
    async testConnection(): Promise<void> {
      ...
      try {
        ...
        const messages = [
          { role: "user" as const, content: "Hello! Reply 'OK'." },
        ];
        ...
        const response = await callLLM(
          ...
          this.llm.model,
          {
            ...
            jsonMode: false,
            timeout:
              configCenter.get<number>("llm.testConnectionTimeout") || 15000,
          },
        );
    ...
    loadProxyConfig(): void {
      const savedConfig = StorageService.get(
        STORAGE_KEYS.SCRAPER_PROXY_CONFIG,
        {},
      ) as { type?: string; customUrl?: string } | null;
      this.proxy.savedKeyMap =
        (StorageService.get(STORAGE_KEYS.PROXY_KEY_MAP, {}) as Record<
          string,
          string
        >) || {};
...
export async function updateModelStatus(): Promise<void> {
  const provider = StorageService.get(STORAGE_KEYS.LLM_ACTIVE_PROVIDER) as
    | string
    | null;
