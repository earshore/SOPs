...
it('keeps the PPC action table named, described, and column-scoped', () => {
  ...
  expect(html).toContain('id="ppc-search-terms-results-title"');
  expect(html).toContain('id="ppc-search-terms-table-wrapper"');
  expect(html).toContain('role="region"');
  expect(html).toContain('aria-labelledby="ppc-search-terms-results-title"');
  expect(html).toContain(
    'aria-describedby="ppc-search-terms-result-count ppc-search-terms-table-help"'
  );
  expect(html).toContain('tabindex="0"');
  expect(html).toContain('id="ppc-search-terms-table-help"');
...
it('keeps Restricted Words lookup tables named and column-scoped', () => {
  ...
  const alternativesHtml = html.slice(
    alternativesStart,
    html.indexOf('<!-- ========== 已上架Listing紧急排查', alternativesStart)
  );
