...
import {
  startScrape,
  saveScrapeSnapshot,
} from '@/modules/app_center/views/master_analysis/scraper/handlers/scrapeHandler';
import {
...
const scraperMocks = vi.hoisted(() => {
  ...
  return {
    ...
    updateTask: vi.fn(
      (
        tasks: Array<{ asin: string; status: string; message: string }>,
        asin: string,
        status: string,
        message: string
      ) => {
...
vi.mock('@/services/storageService', () => ({
  ...
  StorageService: {
    ...
    hasProxyCredential: vi.fn(() => false),
  },
}));
...
it('reads proxy, preview, and store-backed state branches', () => {
  ...
  scraperMocks.storageGet.mockReturnValueOnce({
    type: 'custom_proxy',
    customUrl: 'http://proxy.test',
  });
