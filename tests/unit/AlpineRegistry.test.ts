...
function setupRegistry(
  options: Parameters<typeof AlpineRegistry.getInstance>[0] = { logLevel: 'error' }
): AlpineRegistry {
  resetRegistrySingleton();
  (global as any).window = {
    Alpine: mockAlpine,
  };
  mockAlpine.data.mockReset();
  mockAlpine.start.mockReset();
...
describe('AlpineRegistry - register 方法', () => {
  it('应该成功注册一个简单组件', () => {
...
describe('AlpineRegistry - unregister 方法', () => {
  it('应该成功注销已注册的组件', () => {
    const factory = () => ({ data: 'test' });
...
describe('AlpineRegistry - isComponentRegistered 方法', () => {
  it('应该正确返回组件注册状态', () => {
    const factory = () => ({ data: 'test' });
...
describe('AlpineRegistry - getRegisteredComponents 方法', () => {
  it('应该返回所有已注册组件的名称', () => {
    const factory = () => ({ data: 'test' });
...
describe('AlpineRegistry - 组件注册流程', () => {
...
describe('AlpineRegistry - init 基本功能', () => {
...
describe('AlpineRegistry - autoStart 选项', () => {
...
describe('AlpineRegistry - init 错误处理', () => {
  it('应该处理组件注册失败的情况', () => {
    const factory1 = () => ({ data: 'test1' });
    const factory2 = () => ({ data: 'test2' });
...
describe('AlpineRegistry - 批量注册性能', () => {
  it('应该一次性注册所有组件', () => {
    const factory = () => ({ data: 'test' });
...
describe('AlpineRegistry - 简单依赖解析', () => {
...
describe('AlpineRegistry - 链式依赖解析', () => {
  it('应该处理链式依赖 (A -> B -> C)', () => {
    const factoryA = () => ({ data: 'A' });
    const factoryB = () => ({ data: 'B' });
    const factoryC = () => ({ data: 'C' });
...
describe('AlpineRegistry - 循环依赖检测', () => {
...
describe('AlpineRegistry - 缺失依赖处理', () => {
  it('应该警告但不阻止注册（依赖未找到）', () => {
    const factory = () => ({ data: 'test' });
...
describe('AlpineRegistry - 依赖解析边界情况', () => {
  it('应该处理空依赖数组', () => {
    const factory = () => ({ data: 'test' });
