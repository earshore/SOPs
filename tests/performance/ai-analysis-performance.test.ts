// ================================================================
// 🚀 AI 分析页面性能测试
// 使用 Playwright 和 Lighthouse 进行 AI 分析模块性能测试
// ================================================================
 ...
 * AI 分析是一个计算密集型页面，阈值相对宽松
 */
const PERFORMANCE_THRESHOLDS = {
  // Core Web Vitals
  performance: 85,        // 性能评分 > 85 (计算密集型页面标准)
  ...
  tbt: 500,               // Total Blocking Time < 500ms (计算密集型页面)
...
// ================================================================
  ...
  test('应该满足性能阈值要求', async ({ page, baseURL }) => {
    console.log(`\n🔍 测试页面: AI 分析`);
    console.log(`   URL: /#/app-center/ai-analysis`);
    
    // 导航到 AI 分析页面
    const fullUrl = `${baseURL}/#/app-center/ai-analysis`;
    ...
    // 等待 Alpine 组件加载
    await page.waitForSelector('[x-data*="aiAnalysis"]', { timeout: 10000 }).catch(() => {
    ...
    // 保存报告
    saveLighthouseReport('ai-analysis', audits, 'json');
    ...
    expect(
      performanceScore,
      `AI 分析性能评分应该 >= ${PERFORMANCE_THRESHOLDS.performance}，实际: ${performanceScore.toFixed(1)}`
    ).toBeGreaterThanOrEqual(PERFORMANCE_THRESHOLDS.performance);
    
    expect(
      accessibilityScore,
      `AI 分析可访问性评分应该 >= ${PERFORMANCE_THRESHOLDS.accessibility}，实际: ${accessibilityScore.toFixed(1)}`
    ).toBeGreaterThanOrEqual(PERFORMANCE_THRESHOLDS.accessibility);
    
    expect(
      bestPracticesScore,
      `AI 分析最佳实践评分应该 >= ${PERFORMANCE_THRESHOLDS.bestPractices}，实际: ${bestPracticesScore.toFixed(1)}`
    ).toBeGreaterThanOrEqual(PERFORMANCE_THRESHOLDS.bestPractices);
    
    expect(
      seoScore,
      `AI 分析 SEO 评分应该 >= ${PERFORMANCE_THRESHOLDS.seo}，实际: ${seoScore.toFixed(1)}`
    ...
    expect(
      fcp,
      `AI 分析 FCP 应该 < ${PERFORMANCE_THRESHOLDS.fcp}ms，实际: ${Math.round(fcp)}ms`
    ).toBeLessThan(PERFORMANCE_THRESHOLDS.fcp);
    
    expect(
      lcp,
      `AI 分析 LCP 应该 < ${PERFORMANCE_THRESHOLDS.lcp}ms，实际: ${Math.round(lcp)}ms`
    ).toBeLessThan(PERFORMANCE_THRESHOLDS.lcp);
    
    expect(
      cls,
      `AI 分析 CLS 应该 < ${PERFORMANCE_THRESHOLDS.cls}，实际: ${cls.toFixed(3)}`
    ).toBeLessThan(PERFORMANCE_THRESHOLDS.cls);
    
    expect(
      tbt,
      `AI 分析 TBT 应该 < ${PERFORMANCE_THRESHOLDS.tbt}ms，实际: ${Math.round(tbt)}ms`
  ...
  test('应该验证 LCP < 2.8s', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 AI 分析 LCP: ${formatMetric(lcp)}`);
  ...
  test('应该验证 FID < 100ms (通过 TBT 估算)', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 AI 分析 TBT (FID 估算): ${formatMetric(tbt)}`);
    
    // TBT < 500ms 对于计算密集型页面是合理的
    expect(
      tbt,
  ...
  test('应该验证 CLS < 0.1', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 AI 分析 CLS: ${cls.toFixed(3)}`);
  ...
  test('应该验证 JavaScript 执行时间合理', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 AI 分析 JavaScript 执行时间: ${formatMetric(bootupTime)}`);
    
    // JavaScript 执行时间应该 < 4s (计算密集型页面)
    expect(
      bootupTime,
  ...
  test('应该验证主线程工作时间合理', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 AI 分析主线程工作时间: ${formatMetric(mainthreadWork)}`);
    
    // 主线程工作时间应该 < 5s (计算密集型页面)
    expect(
      mainthreadWork,
  ...
  test('应该验证 TTI < 4s', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 AI 分析 TTI: ${formatMetric(tti)}`);
  ...
  test('应该验证无控制台错误', async ({ page, baseURL }) => {
    ...
    console.log(`\n   🐛 AI 分析控制台错误数: ${consoleErrors.length}`);
