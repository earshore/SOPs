...
import {
  ...
  type ModuleCssConfig,
} from '../config/moduleCssRegistry';
...
class ModuleCssLoader {
  ...
  async preloadHighPriorityModules(): Promise<void> {
    const highPriorityModules = Object.values(MODULE_CSS_REGISTRY).filter(
      config => config.preload && config.priority === 'high'
    );

    await Promise.all(highPriorityModules.map(config => this.preloadModuleCSS(config.moduleId)));
  }
