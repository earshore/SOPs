...
test('应该能够开始采集并显示进度', async ({ page }) => {
  ...
  // 验证：已进入采集流程。采集可能很快完成，不强依赖瞬时 isScraping 状态。
  await expect
    .poll(() => scraper.getTaskCount(), {
      message: '应该创建采集任务',
      timeout: 5000,
    })
    .toBeGreaterThan(0);
  await expect
    .poll(() => scraper.hasTaskStatusPanel(), {
      message: '应该显示采集状态面板',
      timeout: 10000,
    })
    .toBe(true);
...
test.describe('策略指南功能', () => {
  test('应该默认显示策略指南且不提供展开/收起入口', async () => {
    ...
    await expect(scraper.hasStrategyGuideToggle(), '策略指南不应保留展开/收起入口').resolves.toBe(
      false
    );
