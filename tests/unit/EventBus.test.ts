...
describe('错误处理', () => {
  ...
  it('应该限制监听器错误记录数量并记录日志', () => {
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => {});
    let count = 0;
    eventBus.configure({ maxListenerErrors: 2 });
    eventBus.on('test-event', () => {
      count += 1;
    ...
    const errors = eventBus.getListenerErrors();
    expect(errors).toHaveLength(2);
    expect((errors[0]?.error as Error).message).toBe('Error 2');
    expect((errors[1]?.error as Error).message).toBe('Error 3');
