  ...
  const clickDelay = await page.evaluate(() => {
    return new Promise<number>(resolve => {
      const button = Array.from(document.querySelectorAll('button')).find(element =>
        element.textContent?.includes('开始采集')
...
test('多次操作后内存不应持续增长（内存泄漏检测）', async ({ page }) => {
  ...
  const clearInputButton = page
    .locator('.manual-input-card .clear-btn')
    .filter({ hasText: '清空' })
    .first();
...
test.describe('数据采集性能', () => {
  test('单个 ASIN 采集时间应该合理', async ({ page }) => {
    ...
    // 验证：采集时间应该小于 15 秒
    expect(scrapeTime, `单个 ASIN 采集时间应该 < 15000ms，实际: ${scrapeTime}ms`).toBeLessThan(
      15000
    );
  });
  ...
  test('多个 ASIN 采集时间应该合理', async ({ page }) => {
    ...
    // 验证：采集时间应该小于 30 秒
    expect(scrapeTime, `多个 ASIN 采集时间应该 < 30000ms，实际: ${scrapeTime}ms`).toBeLessThan(
      30000
    );
  });
});
