...
const reviewOwnerField = createOwnerField({
  ...
  inputId: 'promotion-submission-owner',
});
...
const copyPromotionSubmissionTemplate = createTemplateCopyAction({
  ownerField: reviewOwnerField,
  buildTemplate: buildPromotionSubmissionTemplate,
  successMessage: '已复制促销提报/复盘模板，可粘贴到周报或归档文档。',
  failureMessage: '复制失败，请手动复制促销提报模板或稍后重试。',
});
...
class PromotionSubmissionModule extends BaseModule {
  ...
  private performCalculation(inputs: PromotionInputs): CalculationResult {
    ...
    const profit =
      salePrice - commission - inputs.fbaFee - vatAmount - inputs.cost - amortizedPromoFee;
  ...
  private updateProfitDisplay(profit: number): void {
    ...
    if (profitBox && warningBox && profitEl) {
      if (profit < 0) {
        ...
        profitBox.className =
          'p-4 bg-emerald-100 rounded-lg border-2 border-emerald-300 text-center';
