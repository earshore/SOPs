...
describe('Competitor Report', () => {
  it('should extract DNA from competitor report', () => {
    const report: Partial<CompetitorReport> = {
      ...
      high_frequency_phrases: ['schöner Klang', 'sehr hübsch', 'gut verarbeitet'],
      feature_points: [
      ...
      competitor_insights: {
        ...
        strengths: ['Klare Positionierung auf lautem Klang', 'Robuste Metallmaterialien'],
        weaknesses: ['Qualitätsinkonsistenz bei Griffbefestigung'],
        differentiation_angles: ['Verbesserte Befestigungsqualität', 'Klangbeschreibung & -beweis'],
      },
      meta: {
        targetMarket: 'German',
    ...
    const dna = expectExtractedDNA(extractor.extractDNA(report));
    expect(dna.keywords.core).toContain('Handglocke');
    expect(dna.keywords.longTail).toContain('Handglocke für Kinder zum Essen rufen');
    expect(dna.highFrequencyPhrases).toContain('schöner Klang');
    expect(dna.audience).toContain('Familien mit Kindern');
    expect(dna.painPoints).toContain('Qualitätsinkonsistenz bei Griffbefestigung');
  ...
  it('should extract DNA from competitor report with camelCase fields', () => {
    const report = {
      ...
      complianceRisks: [{ type: 'claim', examples: ['guaranteed'], suggestion: 'soften claim' }],
      qaOpportunities: [],
      meta: {},
    };
...
describe('Product Overview Report - camelCase', () => {
  it('should extract DNA from product overview report', () => {
    const report: Partial<ProductOverviewReport> = {
      ...
      differentiationAngles: ['Durability-first SKU', 'Performance optimization'],
      keywordClusters: {
...
describe('Product Overview Report - snake_case', () => {
  it('should extract DNA from product overview report with snake_case fields', () => {
    const report = {
      ...
      compliance_risks: [{ type: 'claim', risk: 'eco friendly', suggestion: 'substantiate' }],
      meta: {},
    };
...
describe('Semantic Analysis Report', () => {
  it('should extract DNA from semantic analysis report', () => {
    const report: Partial<SemanticAnalysisReport> = {
      ...
      pain_point_gaps: {
        top_quality_issues: ['Zu hart für einige Katzen', 'Kurzzeitige Wirkung'],
    ...
    expect(dna.differentiationAngles).toEqual(
      expect.arrayContaining([expect.stringContaining('Killer Feature')])
    );
