...
sops 是一个 Vite + TypeScript 前端项目，面向亚马逊运营团队，提供 SOP 流程、Amazon 智库、工作台和大模型探索工具。当前部署形态是 Cloudflare Pages 托管静态资源，浏览器端按用户配置调用 `https://new.hongecb.store/v1` 中转站；仓库和 Pages 项目不应保存生产 API key。
...
- **深色可读性深度审计**：More 子页与系统设置深层页面（网络设置 / 系统设置 / 卡片 / 探索页 / Prompt 页）逐元素对比度修复。
- **图标可读性专项**：sidebar / welcome-banner / AMZ Hub 营销日历·促销活动·促销工具·质量 Listing / App Center / PPC 搜索词 hero / SOPS 流程中心 10 页面图标与文本审计归零。
- **审计工具 local-contrast 合成修正 + 全量复检**：按背景叠层合成计算实际对比，40 路由 × dark/light 双模式全部 0 缺陷（含 toast 通知、Video Resource 备注等区域）。
- **settings-scale 门禁修复**：`systemSettings.css` 1230 → 1195 行（注释收敛 + 重复块合并，无样式变更）。
- GitHub Latest **仍指向** `v3.1.1`；回滚基线为 `v3.1.1`；生产目标 `https://sops.hongecb.store`。
...
- **统一加载视觉**：页面级转场动画整体移除（性能治理），新增路由加载点状指示器 `MinimalLoadingIndicator`；工作台 / PPC 等页面进入改为静态直达。
- **深色模式修复**：三处遗留深色缺口修复（modal / badge / form 暗色覆盖）。
- **AI 分析提示词常量化**：主分析 system prompt 提取为 `MASTER_ANALYSIS_SYSTEM_PROMPT` 常量（`analysisPrompts.ts`）；Map-Reduce 管线全部 15 处内联模板提取为 `pipelinePrompts.ts` 常量与工厂函数，消除重复并统一插值上下文；`ci:quality` 与 smoke 30/30 回归通过。
...
| 工作台        | Master Analysis、PPC Tools、Keyword Hunter、Deep Chat Playground | `src/modules/app_center/` |
