...
describe('Brand infringement review workflow', () => {
  ...
  it('copies the review template and records local usage', async () => {
    ...
    expect(loadTemplate).toHaveBeenCalledWith('src/modules/sops/views/safety/brand_infringement/template.html');
    ...
    expect(StorageService.set).toHaveBeenCalledWith('ops_metrics_v1', expect.objectContaining({
      'brand_infringement.review_template_copy': expect.objectContaining({ count: 1 }),
    }));
