...
### 5. 添加置信度筛选功能 (任务 #5)
- ✅ 添加了 `confidenceFilter` 状态
  - `showHighOnly`: 仅显示高置信度 (≥70%)
  - `hideLow`: 隐藏低置信度 (<50%)
- ✅ 添加了 `shouldShowTarget(targetId)` 方法 - 判断是否显示
- ✅ 添加了 `onConfidenceFilterChange()` 方法 - 处理筛选变化
- ✅ 在 `renderNewFormatModules` 中应用筛选逻辑
