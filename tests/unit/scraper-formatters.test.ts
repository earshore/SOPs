...
describe('formatDate - 格式化日期', () => {
  ...
  it('应该统一为 MM/DD HH:mm 格式', () => {
    const mockNow = new Date('2024-01-15T14:30:00');
    vi.setSystemTime(mockNow);

    const todayLocal = new Date(2024, 0, 15, 10, 25, 0);
    const result = formatDate(todayLocal.toISOString());

    expect(result).toBe('01/15 10:25');

    vi.useRealTimers();
  });

  it('应该对跨日时间补零显示 MM/DD HH:mm', () => {
    const mockNow = new Date('2024-01-15T14:30:00');
    vi.setSystemTime(mockNow);

    const yesterdayLocal = new Date(2024, 0, 14, 9, 5, 0);
    const result = formatDate(yesterdayLocal.toISOString());

    expect(result).toBe('01/14 09:05');
  ...
  it('应该正确格式化时间（补零）', () => {
    ...
    expect(result).toBe('01/15 09:05');
  ...
  it('应该处理不同月份的日期', () => {
    ...
    const christmas = new Date(2023, 11, 25, 10, 0, 0);
    const result = formatDate(christmas.toISOString());

    expect(result).toBe('12/25 10:00');
