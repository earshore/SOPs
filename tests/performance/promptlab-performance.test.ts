// ================================================================
// 🚀 Promptlab 页面性能测试
// 使用 Playwright 和 Lighthouse 进行 Promptlab 模块性能测试
// ================================================================
 ...
 * Promptlab 是一个交互密集型页面，阈值相对宽松
 */
const PERFORMANCE_THRESHOLDS = {
  // Core Web Vitals
  performance: 85,        // 性能评分 > 85 (交互页面标准)
  ...
  tbt: 500,               // Total Blocking Time < 500ms (交互页面)
...
function printPerformanceSummary(audits: any) {
  console.log(`\n${'='.repeat(70)}`);
  console.log(`📊 Promptlab 页面性能报告摘要`);
  ...
  const tti = audits.audits['interactive']?.numericValue || 0;
  ...
  // 资源统计
  console.log('\n📦 资源统计:');
  const resourceSummary = audits.audits['resource-summary']?.details?.items || [];
  ...
  // JavaScript 执行时间
  const bootupTime = audits.audits['bootup-time']?.numericValue || 0;
  console.log(`\n⚡ JavaScript 执行时间: ${formatMetric(bootupTime)}`);
  
  // 主线程工作
  const mainthreadWork = audits.audits['mainthread-work-breakdown']?.numericValue || 0;
...
test.describe('Promptlab 页面性能测试', () => {
  ...
  test('应该满足性能阈值要求', async ({ page, baseURL }) => {
    console.log(`\n🔍 测试页面: Promptlab`);
    console.log(`   URL: /#/app-center/promptlab`);
    
    // 导航到 Promptlab 页面
    const fullUrl = `${baseURL}/#/app-center/promptlab`;
    ...
    // 等待 Alpine 组件加载
    await page.waitForSelector('[x-data*="promptlab"]', { timeout: 10000 }).catch(() => {
    ...
    // 保存报告
    saveLighthouseReport('promptlab', audits, 'json');
    ...
    expect(
      performanceScore,
      `Promptlab 性能评分应该 >= ${PERFORMANCE_THRESHOLDS.performance}，实际: ${performanceScore.toFixed(1)}`
    ).toBeGreaterThanOrEqual(PERFORMANCE_THRESHOLDS.performance);
    
    expect(
      accessibilityScore,
      `Promptlab 可访问性评分应该 >= ${PERFORMANCE_THRESHOLDS.accessibility}，实际: ${accessibilityScore.toFixed(1)}`
    ).toBeGreaterThanOrEqual(PERFORMANCE_THRESHOLDS.accessibility);
    
    expect(
      bestPracticesScore,
      `Promptlab 最佳实践评分应该 >= ${PERFORMANCE_THRESHOLDS.bestPractices}，实际: ${bestPracticesScore.toFixed(1)}`
    ).toBeGreaterThanOrEqual(PERFORMANCE_THRESHOLDS.bestPractices);
    
    expect(
      seoScore,
      `Promptlab SEO 评分应该 >= ${PERFORMANCE_THRESHOLDS.seo}，实际: ${seoScore.toFixed(1)}`
    ...
    expect(
      fcp,
      `Promptlab FCP 应该 < ${PERFORMANCE_THRESHOLDS.fcp}ms，实际: ${Math.round(fcp)}ms`
    ).toBeLessThan(PERFORMANCE_THRESHOLDS.fcp);
    
    expect(
      lcp,
      `Promptlab LCP 应该 < ${PERFORMANCE_THRESHOLDS.lcp}ms，实际: ${Math.round(lcp)}ms`
    ).toBeLessThan(PERFORMANCE_THRESHOLDS.lcp);
    
    expect(
      cls,
      `Promptlab CLS 应该 < ${PERFORMANCE_THRESHOLDS.cls}，实际: ${cls.toFixed(3)}`
    ).toBeLessThan(PERFORMANCE_THRESHOLDS.cls);
    
    expect(
      tbt,
      `Promptlab TBT 应该 < ${PERFORMANCE_THRESHOLDS.tbt}ms，实际: ${Math.round(tbt)}ms`
  ...
  test('应该验证 LCP < 2.8s', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 Promptlab LCP: ${formatMetric(lcp)}`);
  ...
  test('应该验证 FID < 100ms (通过 TBT 估算)', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 Promptlab TBT (FID 估算): ${formatMetric(tbt)}`);
    
    // TBT < 500ms 对于交互密集型页面是合理的
    expect(
      tbt,
  ...
  test('应该验证 CLS < 0.1', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 Promptlab CLS: ${cls.toFixed(3)}`);
  ...
  test('应该验证 JavaScript 执行时间合理', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 Promptlab JavaScript 执行时间: ${formatMetric(bootupTime)}`);
    
    // JavaScript 执行时间应该 < 4s (交互密集型页面)
    expect(
      bootupTime,
  ...
  test('应该验证主线程工作时间合理', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 Promptlab 主线程工作时间: ${formatMetric(mainthreadWork)}`);
    
    // 主线程工作时间应该 < 5s (交互密集型页面)
    expect(
      mainthreadWork,
  ...
  test('应该验证 TTI < 4s', async ({ page, baseURL }) => {
    ...
    console.log(`   📊 Promptlab TTI: ${formatMetric(tti)}`);
  ...
  test('应该验证无控制台错误', async ({ page, baseURL }) => {
    ...
    const consoleErrors = audits.audits['errors-in-console']?.details?.items || [];
    
    console.log(`\n   🐛 Promptlab 控制台错误数: ${consoleErrors.length}`);
