...
import {
  NavigoAdapter,
  createRouter,
} from '@/common/router/navigo/NavigoAdapter';
import type {
  RouteConfig,
  RouteMiddleware,
  RouterStoreSync,
...
const navigoMock = vi.hoisted(() => {
  ...
  const ctor = vi.fn(function MockNavigo(
    this: MockNavigoInstance,
    root: string,
    options: unknown
  ) {
    this.root = root;
...
afterEach(() => {
  ...
  it('rejects invalid route configs and navigate options', async () => {
    ...
    await expect(
      adapter.navigate('/orders', { state: [] } as never)
  ...
  it('navigates through middleware, updates history, and syncs store state', async () => {
    ...
    await expect(
      adapter.navigate('/orders', { state: { source: 'test' } })
    ).resolves.toBe(true);
