...
const panelMocks = vi.hoisted(() => {
  ...
  return {
    ...
    createMultipleStateSyncs: vi.fn((configs: Array<{ selector: (state: typeof storeState) => unknown; onChange: (value: unknown) => void; immediate?: boolean }>) =>
      configs.map((config) => {
        if (config.immediate) {
    ...
    helperGetResultColor: vi.fn((targetId: string) => (targetId.includes('review') ? 'amber' : 'blue')),
...
vi.mock('@/modules/app_center/views/master_analysis/ai_analysis/components/PerformanceSettings', () => ({
...
beforeEach(() => {
  ...
  it('computes selection, target, prompt, and visual class branches', () => {
    ...
    expect(panel.getAsinOptionClass('B001')).toContain('bg-indigo-50');
    expect(panel.getAsinOptionClass('B002')).toContain('hover:border-indigo-200');
    expect(panel.getListingTargetCardClass('selling-points')).toContain('border-blue-300');
    ...
    expect(panel.getTargetCheckClass('selling-points')).toContain('bg-indigo-500');
  ...
  it('computes hero, progress, report, JSON, and confidence branches', () => {
    ...
    expect(panel.runAnalysisButtonClass).toContain('bg-indigo-600');
    ...
    expect(panel.getHighlightClass('danger')).toHaveProperty('bg-gradient-to-r from-red-50 to-rose-50 text-red-700 border-red-200', true);
  ...
  it('computes idle, disabled, fallback, and confidence edge branches', () => {
    ...
    expect(panel.getAsinTextClass('B001')).toBe('text-indigo-700');
    ...
    expect(panel.getListingTargetIconWrapClass('selling-points')).toContain('group-hover:bg-slate-200');
    ...
    panelMocks.helperGetResultColor.mockReturnValueOnce('violet');
    expect(panel.getResultCategoryClass('unknown')).toBe('bg-blue-50 text-blue-700 border border-blue-100');
    panelMocks.helperGetResultColor.mockReturnValueOnce('violet');
