...
describe("release web assets", () => {
  ...
  it("keeps robots.txt out of the Vercel SPA fallback", () => {
    ...
    const robotsRewrite = vercel.rewrites.find(
      (rewrite) => rewrite.source === "/robots.txt",
    );
    ...
    expect(spaFallback).toBeUndefined();
  });

  it("aligns Vercel clean-path redirects with Cloudflare hash-canonical contract", () => {
    const vercel = JSON.parse(read("vercel.json")) as {
      redirects: Array<{
        destination: string;
        source: string;
    ...
    const required = [
      ...
      { source: "/app-center", destination: "/#/app-center" },
      { source: "/app-center/:path*", destination: "/#/app-center/:path*" },
      { source: "/sops", destination: "/#/sops" },
      { source: "/sops/:path*", destination: "/#/sops/:path*" },
      { source: "/amz-hub", destination: "/#/amz-hub" },
      { source: "/amz-hub/:path*", destination: "/#/amz-hub/:path*" },
      { source: "/more", destination: "/#/more" },
      { source: "/more/:path*", destination: "/#/more/:path*" },
      { source: "/sops_:path*", destination: "/#/sops_:path*" },
      { source: "/amz_:path*", destination: "/#/amz_:path*" },
      { source: "/more_:path*", destination: "/#/more_:path*" },
    ];

    for (const rule of required) {
      const match = vercel.redirects.find(
        (item) => item.source === rule.source,
      );
      expect(match, `missing Vercel redirect for ${rule.source}`).toBeDefined();
      expect(match?.destination).toBe(rule.destination);
      expect(match?.statusCode).toBe(302);
    }
  ...
  it("hides initial main content until route layout is stable", () => {
    ...
    expect(indexHtml).toMatch(
      /id="main-content"[\s\S]*?class="[^"]*app-shell-pending/,
    );
    expect(criticalCss).toMatch(
      /\.app-shell-pending\s*{[^}]*visibility:\s*hidden/s,
    );
  });
  ...
  it("waits for the home view and main styles before the initial reveal", () => {
    ...
    if (startupListeners.length !== 1) {
      throw new Error(
        `Expected one DOMContentLoaded listener, found ${startupListeners.length}`,
      );
    }
    ...
    if (homeWaitBranches.length !== 1) {
      throw new Error(
        `Expected one home wait branch, found ${homeWaitBranches.length}`,
      );
    }
    ...
    if (
      ...
      !readyPromises ||
      !ts.isArrayLiteralExpression(readyPromises)
    ) {
    ...
    if (
      ...
      throw new Error(
        "Promise.all must await homeViewReady then mainStylesReady",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Home wait branch must contain one revealInitialHomeView block",
      );
    }

    const directCallNames = revealBranch.thenStatement.statements.flatMap(
      (statement) => {
        if (
    ...
    expect(directCallNames).toEqual([
  ...
  it("keeps unrelated domain registration failures from aborting startup", () => {
    ...
    expect(startup).not.toContain(
      "const domainModulesReady = loadDomainModules().catch(",
    );
  });
