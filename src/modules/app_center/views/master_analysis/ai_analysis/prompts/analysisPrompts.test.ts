...
describe('generateAnalysisPrompt - validation', () => {
  ...
  it('should validate required product fields', () => {
    ...
    expect(() => {
      generateAnalysisPrompt('title-keywords', {
        ...invalidProduct,
        productTitle: '',
      });
    ...
    expect(() => {
      generateAnalysisPrompt('title-keywords', {
        ...invalidProduct,
        productTitle: 'Test',
        customer_reviews: 'not-array',
      } as unknown as ProductArgument);
    ...
    expect(() => {
      generateAnalysisPrompt('title-keywords', {
        ...
        productTitle: 'Test',
        customer_reviews: [],
        feature_bullets: 'not-array',
      } as unknown as ProductArgument);
    }).toThrow('产品对象的 feature_bullets 必须是数组');
  });
});

describe('generateAnalysisPrompt - language handling', () => {
  ...
  it('should warn for unusual language codes', () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(expect.stringContaining('Unusual language code: xx'));

    consoleSpy.mockRestore();
  });
});

describe('generateAnalysisPrompt - sanitization and reviews', () => {
  ...
  it('should expose review sampling metadata for report transparency', () => {
    const product = {
      ...mockProduct,
      customer_reviews: Array.from({ length: 50 }, (_, index) => ({
        body: `Review body ${index}`,
        headline: `Review headline ${index}`,
        origin_country: 'US',
        review_date: '2024-01-01',
        star_rating: index < 30 ? 2 : 5,
        _origin_site: 'US',
      })),
    };

    const metadata = getReviewSamplingMetadata(product);

    expect(metadata.totalReviews).toBe(50);
    expect(metadata.lowStar).toMatchObject({
      totalReviews: 30,
      includedReviews: 24,
      omittedReviews: 6,
    });
    expect(metadata.highStar).toMatchObject({
      totalReviews: 20,
      includedReviews: 20,
      omittedReviews: 0,
    });
    expect(metadata.general).toMatchObject({
      totalReviews: 50,
      includedReviews: 40,
      omittedReviews: 10,
