...
export class CompetitorReportAdapter implements ReportAdapter {
   ...
   * 提取限制词
   */
  private extractRestrictedWords(report: CompetitorReport): ExtractionResult<string[]> {
  ...
  private extractPainPoints(report: CompetitorReport): ExtractionResult<string[]> {
    ...
    // 从 weaknesses 提取
    if (
      report.competitor_insights?.weaknesses &&
      report.competitor_insights.weaknesses.length > 0
    ) {
