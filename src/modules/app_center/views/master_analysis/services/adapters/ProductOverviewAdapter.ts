...
export class ProductOverviewAdapter implements ReportAdapter {
  ...
  private normalizeReport(report: unknown): ProductOverviewReport {
    ...
    return {
      ...
      productOverview: (reportObj.productOverview || reportObj.product_overview || {}) as any,
      coreFeatures: (reportObj.coreFeatures || reportObj.core_features || {}) as any,
      user_profile: {
        demographics: {
          age_ranges: (demographics.age_ranges || demographics.ageRanges || []) as string[],
        ...
        pain_points: ((userProfile as any).pain_points || (userProfile as any).painPoints || []) as string[],
        scenarios: ((userProfile as any).scenarios || []) as string[],
        objections: ((userProfile as any).objections || []) as string[],
        price_sensitivity: ((userProfile as any).price_sensitivity || (userProfile as any).priceSensitivity || '') as string,
        decision_drivers: ((userProfile as any).decision_drivers || (userProfile as any).decisionDrivers || []) as string[]
      },
      strengths: (reportObj.strengths || []) as string[],
      weaknesses: (reportObj.weaknesses || []) as string[],
      differentiationAngles: (reportObj.differentiationAngles || reportObj.differentiation_angles || []) as string[],
      keywordClusters: (reportObj.keywordClusters || reportObj.keyword_clusters || {
        core: [],
        longTail: [],
        intent: []
      }) as any,
      complianceRisks: (reportObj.complianceRisks || reportObj.compliance_risks || []) as any[]
    };
  }
}
