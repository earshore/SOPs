...
| **5** D6 业务页分期            | 业务 `blue-*` 降 | **Partial（Phase B-2/B-3 已闭环）**                 | 业务 ~900+ 基线 → **sops 377 / app_center 39 / amz_hub 175 / more 40**；**样本 through #16 + residual #18–27**: KH…#9 + **#10 SOPs** + **#11 NPI** + **#12 Restricted Words** + **#13 AMZ Hub nav**（ownership orange / `module-accent`）+ **#14 email_templates** + **#15 qa_maintenance** + **#16 PromptLab DNA/autoPopulate** + **#18 Scraper / #19 AI / #20 PPC residual** @ `a05b12c8` + **#21 PromptLab residual focus** @ `f761e4e5` + **#22 SafeModuleLoader primary CTA** @ `1711eec1` + **#23 Home splash/floating CTA** @ `cbad435b` + **#24 confirmModal primary chrome** @ `3b947dc6` + **#25 AppModal title/accent soft chrome** @ `fb5384d6` + **#26 import-conflict recommended badge/icon soft chrome** @ `77fd0cdc` + **#27 shared legacy-compat primary CTA + search focus** + **Phase B 换肤迁移** @ `8a04ea9b`+`1f9a1572`+`c32bedb9`；Skills #6 / Deep Chat #7；phase/tab/step 多色与 SEO 卡 **保留**；壳层 megaMenu **13**；**B-4 已闭环** @ `601311d3`（sops 377→334）；**B-5 已闭环** @ `677fc656`（amz_hub 175→162：eu_insights/seo/cvo/mature/np30 单色 section icon；quality_listing 双色体系、more 页面主题色、卡组/徽章/指标组全部复核保留）；**B-6 已终审** @ `B-6`：app_center 39 为 8 色用户强调色映射表成员（保留合理） |
...
| 业务 `blue-*`（D6）              | ~900+ 行级基线                          | Phase 5；**样本 through #16 + residual #18–27**: KH…#9 + **#10 SOPs** + **#11 NPI** + **#12 Restricted Words** + **#13 AMZ Hub nav** + **#14 email_templates** + **#15 qa_maintenance** + **#16 PromptLab DNA** + **#18–20 residual MA/PPC** @ `a05b12c8` + **#21 PromptLab residual focus** @ `f761e4e5` + **#22 SafeModuleLoader** @ `1711eec1` + **#23 Home splash/floating CTA** @ `cbad435b` + **#24 confirmModal** @ `3b947dc6` + **#25 AppModal** @ `fb5384d6` + **#26 import-conflict** @ `77fd0cdc` + **#27 legacy-compat** @ `1731ed47` + **Phase B-2 换肤迁移**: sops/app_center `@5891348+8a04eba` + amz_hub/more `@1f9a157` + **B-3 收口**（sops）`@本次`；当前基线 **sops 334 / app_center 39 / amz_hub 162 / more 40**；保留项多为并列色板（指标卡组/步骤徽章/风险等级/深色面浅色字）；`--shadow-primary*` 跟 primary；**非**全量清零；不阻塞 code Green。开外跟踪：**release-smoke 12 例失败**（vite dev 下 CSS import 以 text/javascript 返回、断言按 preview 语义）与 **theme matrix ownership 断言过期**（wb-theme-rose → 实为 cyan）**均已修复**（见 release-smoke / theme-matrix 修复 commit）；B-4/B-5/B-6 已闭环（见行45）；剩余 blue/indigo 均为页面主题色/多色卡组/徽章/指标组/用户强调色映射，收口完成 |
...
| **D7**  | P1        | **Done（deprecate + ESLint + call-site audit）** | `setModuleColor` **@deprecated** + unit + ESLint hard-gate；**0** production callers（§5.3）；dual-channel 兼容层（`getModuleColor`/`onThemeChange`）**有意保留**（`ColorContext.ts` 注释：retire 前勿删）；彻底下线 **不执行**（遗留读路径有测试覆盖） |                                                                      |
| **D8**  | P1        | **Closed（评估：不执行全站绑定）** | Role 表 + **`ownershipRoles.ts`**（19 role + module 映射 + soft tests 全绿）；**全站 `data-ownership-role` 绑定评估（2026-08-07）**：src 中该属性 **0 CSS/JS 消费者**，绑定将产生 wb-theme-* 与 data-role 双源漂移；**结论：先消费者后基础设施**，待 role 级 CSS 需求（如 role 色板覆盖）再立项 |
...
| **D12** | P2 | **Baseline minted → 已人工目检 → 快照已清理** | `theme-appearance-scaffold`：**12 屏 × 2 × 3 browsers = 144 张基线已 mint**（含 settings 打开竞态修复）；terracotta send / Skills violet / NPI growth **不**当 primary；`test:visual:theme:update` 全绿；**不** visual Pass；**待人工 XO 签收**（含 D3 视觉签联动） |
...
- D6 #26 import-conflict recommended badge/icon soft chrome @ `77fd0cdc`
- D6 #27 shared legacy-compat primary CTA + search focus @ `1731ed47`
- D12 scaffold **24**（+ PromptLab + Skills + NPI）@ `023b8952`；first-8 human capture readiness @ `8f13db32`
- D8 soft sop ops / hub advanced ownership @ `adbe0afb`
- XO residual #24–26 Console open tips @ `3e600716`
