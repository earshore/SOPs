...
function createProduct(options: { reviewCount?: number; asinCount?: number; bodyLength?: number; bulletCount?: number } = {}): Product {
  const {
    reviewCount = 4,
    asinCount = 1,
    bodyLength = 80,
...
describe('analysisScheduler', () => {
  ...
  it('prioritizes cached targets and limits concurrency to uncached work', () => {
    ...
    expect(new Set(plan.taskOrder.slice(0, 2))).toEqual(new Set(['promise-reality', 'title-keywords']));
