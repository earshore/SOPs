...
- `v3.1.1` 系列 RC 序列（2026-08-15 重建）：`v3.1.1-rc.1`（6dc3e0a3）/`rc.2`（d6237dd3）/`rc.3`（03833972）/`rc.4`（1a71c3a1）/`rc.5`（c017af8e）均按真实锚点重建 release + tag + 产物。遗留问题：**rc.4 锚点 1a71c3a1 处 package.json version 停留在 3.1.1-rc.3（发布期间未 bump，历史不可变）**；交付物 version 由打包脚本 build-info.json 正确生成（3.1.1-rc.4），无用户可见影响。后续发版须先执行 `npm version` bump 再发 release。
...
- `v3.1.2-rc.1`（2026-08-18）为当前 Pre-release 候选：深色/浅色可读性债务收官——More 子页与系统设置深度审计、图标可读性专项、审计工具 local-contrast 合成修正后 40 路由 × dark/light 双模式 0 缺陷、settings-scale 门禁修复；GitHub Latest 仍为 `v3.1.1`，回滚基线 `v3.1.1`。
...
- `v3.1.0-rc.1`（2026-08-09）为生产验证候选：AI 分析取消与长任务运行感知、总览优化；**Pre-release**，Latest 仍为 `v3.0.12`。
...
4. 不改变 Latest：仅当前稳定 GA（现为 `v3.0.11`）使用 `--latest`；RC 保持 `--prerelease`。
