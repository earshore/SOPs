...
export function getAnalyzableReportText(
  ...
  if (!cleanText) {
    getTextarea(container, 'ppc-search-terms-paste-input')?.focus();
    setPasteInputError(container, '请先粘贴报表内容或选择报表文件。');
    setPpcSearchTermsStatus(
      container,
      '没有可分析的数据，请先粘贴报表内容或选择报表文件。',
      'error'
    );
  ...
  if (cleanText.length > MAX_REPORT_DATA_CHARS) {
    ...
    setPpcSearchTermsStatus(container, message, 'error');
    showToast('报表数据过大', {
      type: 'warning',
...
export function setPasteInputError(container: HTMLElement, message: string): void {
  ...
  const error = getElement(container, 'ppc-search-terms-paste-error');
