...
export class SemanticAnalysisAdapter implements ReportAdapter {
  ...
  private extractKeywords(report: SemanticAnalysisReport): ExtractionResult<{
    ...
    if (
      report.high_frequency_phrases?.attribute &&
      report.high_frequency_phrases.attribute.length > 0
    ) {
    ...
    if (
      report.high_frequency_phrases?.use_cases &&
      report.high_frequency_phrases.use_cases.length > 0
    ) {
  ...
  private extractPainPoints(
    ...
    if (
      report.pain_point_gaps?.top_quality_issues &&
      report.pain_point_gaps.top_quality_issues.length > 0
    ) {
