...
export default [
    ...
    rules: {
      ...
      'no-restricted-syntax': [
        'warn',
        {
          ...
          message:
            '避免直接使用innerHTML。请使用textContent或setSafeHtml()函数，或添加安全注释说明原因。',
        },
        {
          ...
          patterns: [
            {
              group: ['**/loggerService'],
              message:
                '基础设施服务（ConfigCenter, typeGuards, menuConfig等）不应依赖Logger，请使用console',
            },
          ],
        },
      ],
  ...
  {
    // Test code exercises error paths and fixture shapes that are intentionally
    // unlike production code. Keep correctness rules enabled while relaxing
    // production-only complexity and unused-fixture checks.
    files: [
  ...
  {
    // Keep root tests out of the production TypeScript project.
    files: ['tests/**/*.ts'],
    languageOptions: {
      parserOptions: {
