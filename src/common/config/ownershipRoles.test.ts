...
describe('ownershipRoles SSOT (D8 scaffold)', () => {
  ...
  it('maps known modules to roles and palettes', () => {
    ...
    expect(getPaletteForRole('role-keywords')).toBe('cyan');
  ...
  it('soft-cross-checks Sidebar theme class for keyword_hunter with ownership cyan (no production wire)', () => {
    ...
    expect(menuThemeColor).toBe('cyan');
    expect(`sidebar-theme-${palette}`).toBe('sidebar-theme-cyan');
  });
  ...
  it('lists preferred wb-theme classes for roles (keyword_hunter unified cyan)', () => {
    const keywordsClasses = getWbThemeClassesForRole('role-keywords');
    expect(keywordsClasses).toContain('wb-theme-cyan');
