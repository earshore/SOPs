...
describe('deep chat page defaults', () => {
  ...
  it('sanitizePageDefaults drops malformed values and clamps temperature', () => {
    ...
    // 模型不属于页面默认层，旧存储值会被丢弃。
    expect(sanitizePageDefaults({ model: ' gpt-5.5 ' })).toBeNull();
  });
  ...
  it('drops legacy page-default model values while preserving non-model defaults', () => {
    expect(
      sanitizePageDefaults({
        model: 'gpt-5.5',
        modelFingerprint: 'gpt-4.1',
        temperature: 0.7,
      })
