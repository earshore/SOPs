...
- **release.yml 编译修复**：`production-probe` job 级 `if` 引用 `secrets` 上下文导致 workflow 编译失败（rc.6 收口期间任何 push/tag 触发均即时失败，发版阻塞），改为 job env + step 级 `if` 判断（`secrets` 不适用于 job 级 `if`）。
...
- **AI 分析取消机制**：进度区「取消分析」→ 确认弹窗（可记忆）→ AbortController 贯穿编排层
  到每一个 LLM 调用（含 shared Map 与恢复轮）；取消后保留已完成维度、清除断点、
  toast 反馈；二次确认后可整轮取消，不再需要等 deep 档 8-15 分钟自然结束。
- **长任务运行感知**：分析中页签标题「分析中 a/b · 原标题」；总览最近作业展示「分析进行中
  a/b」卡片（完成/取消/失败/数据变更自动移除）；离开页面提示「分析仍在后台进行，
  已完成的维度会自动保存」+ 查看进度入口。
...
- 耗时展示收敛：证据深度下拉不再显示时间（仅档位+推理等级），测算区间只在 start toast；
  Keyword Hunter 启动 toast 改读全局推理档位（与真实调用一致）；同值区间文案收起
  （「约 1-1 秒」→「约 1 秒」）；去掉「预计 约」双重约。
...
- 孤儿估算服务 `estimateRunPlan.ts` 及其测试（生产零引用，-152 行）。

## [3.0.12] - 2026-08-08

> 正式 GA。发布后 GitHub Latest 指向 `v3.0.12`；上一 GA 与生产回滚基线均为 `v3.0.11`。
> 生产目标为 `https://sops.hongecb.store`。
> 本版定稿 `v3.0.12-rc.1`…`rc.10` 的全部候选增量：AI 智能分析推理联动与动态耗时估算、
> Deep Chat 正文净化与流式去重、数据采集导入双模式（合并/导入新的）、LLM 模型选择组件统一，
> 以及上线前门禁加固（静态产物契约、体积门禁、业务 e2e 进 CI）。
...
- 历史快照双存储（localStorage 镜像 + IndexedDB）双向同步：清空/写入不再残留不一致状态。
...
- 流式推理预算：纯推理流不再滑动全量超时，独立预算 max(2×超时,120s) 兜底，
  深度思考不会被误杀。

### Fixed（修复）

- Playground Deep Chat “刷新模型配置”改为真正重新请求 `/models`，而非仅重读本地配置。
- 评论证据渲染上限 50 条/产品卡，避免超长报告拖慢渲染。
- XSS 扫描盲区：query 接收方模式补漏；浏览器基线声明 + `at()` 降级。
- 历史快照样式/术语对齐（合并导入、导入新的）；模板断言防回归。
